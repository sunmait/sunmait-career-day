...
export class CareerDayController extends BaseHttpController {
  ...
  private async getNearestCareerDays(@response() res: express.Response): Promise<void> {
  ...
  private async add(
    ...
    const user = this.httpContext.user as Principal;
    if (!body.UnitManagerId || !body.EmployeeId || user.details.id !== body.UnitManagerId) {
      throw { status: 403 };
    }

    // Checks if an employee already has active career day
    const activeCareerDay = await this._careerDayService.getActiveCareerDay(body.EmployeeId);
    if (activeCareerDay) {
      throw { status: 403 };
    }
  ...
  private async delete(
    ...
    const user = this.httpContext.user as Principal;
    if (careerDayToDelete.UnitManagerId !== user.details.id || !careerDayToDelete.Archived) {
      throw { status: 403 };
    }
