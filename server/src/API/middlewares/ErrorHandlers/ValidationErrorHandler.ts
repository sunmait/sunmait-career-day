...
function isValidationError(err: IValidationError | IErrorAPIWithMessage): err is IValidationError {
...
function ValidationErrorHandler(
  ...
  if (isValidationError(err)) {
    ...
    const errors: { [key: string]: string } = err.errors.reduce(
      (result, errorItem) => ({
        ...result,
        [errorItem.path as string]: `${errorItem.message}. Value ${errorItem.value} is not valid`,
      }),
      {},
    );
