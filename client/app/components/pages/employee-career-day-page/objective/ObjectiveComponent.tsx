...
class Objective extends React.Component<IProps, IState> {
  ...
  private handleChangeValue(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const newState = { [propName as any]: e.target.value } as Pick<IState, stateKeys>;
  ...
  private objectivePanelDetails() {
    ...
    return (
      <Grid container justify="center" alignItems="center" spacing={8}>
        ...
        <Grid item xs={12}>
          <LinearProgress color="primary" value={this.props.objective.Progress * 100} variant="determinate" />
        </Grid>
        <Grid item xs={12}>
          <Typography color="textSecondary" align="right">
            {`Progress: ${Number(this.state.Progress)}/100`}
